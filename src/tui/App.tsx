...
import { TranslationContext, translator } from './i18n'
...
export function App({ ctx, session }: { ctx: IPicGo; session: TuiSession }) {
  ...
  const t = useMemo(() => translator(ctx), [ctx, language])
  const actions = useMemo(() => createActions(ctx), [ctx, language])
  const { exit } = useApp()
  ...
  const items = findActions(actions, sections[section], search, t)
  ...
  useEffect(() => {
    ...
    void uploaderReady(ctx).then(ready => {
      if (!active) return
      setConfigured(ready)
      if (initialReadiness.current) {
        setHighlight(ready ? 0 : 2)
        initialReadiness.current = false
      }
    })
    return () => {
      active = false
    }
  }, [ctx, state.busy, uploader, language])
  useEffect(() => {
  ...
  const start = (index: number) => {
    ...
    setShowError(false)
    void session.run(t(item.action), item.run)
  }
  useInput((input, key) => {
    // A slow plugin validator must not move the cursor after the user starts navigating.
    initialReadiness.current = false
    if (key.ctrl && input === 'c') {
      if (state.busy) {
        setQuitting(true)
    ...
    <TranslationContext.Provider value={t}>
      <Box flexDirection='column' paddingX={1} width={columns}>
        <Box justifyContent='space-between'>
          ...
          {columns >= 76 && <Text color={theme.muted}>{t('Your images, delivered.')}</Text>}
        ...
        <Text wrap='truncate-end'>
          <Text color={theme.muted}>{t('Destination')} </Text>
          {configured ? `${uploader} / ${configName}` : t('Not configured')}
          <Text color={theme.muted}> · {t('Backup')} </Text>
          {secondary}
        </Text>
        <Box marginTop={1} marginBottom={1}>
          {state.busy || showResults ? (
            <Text color={theme.muted}>
              {state.busy ? state.title : `${state.resultTitle || t('Latest operation')} / ${t('Results')}`}
            </Text>
          ) : (
            sections.map((name, index) => (
              <Box key={name} marginRight={columns < 60 ? 1 : 2}>
                ...
                >
                  {index + 1} {t(columns < 55 ? ['Upload', 'Dest.', 'Process', 'Settings'][index] : name)}
                </Text>
              </Box>
            ))
          )}
        ...
        >
          {showError && !state.busy ? (
            <Box flexDirection='column' gap={1}>
              ...
              <Text color={theme.muted}>
                {t('Return to the workspace to check your inputs or destination settings.')}
              </Text>
              <Box>
                <Shortcut keys='Enter' label={t('back to workspace')} />
                {state.results.length > 0 && <Shortcut keys='r' label={t('recent results')} />}
              </Box>
            </Box>
          ) : state.prompt ? (
            <PromptForm
            ...
            <ResultPanel
              ...
              title={state.resultTitle || t('Results')}
              wide={wide}
              width={columns - 6}
            ...
            <Box flexDirection='column' gap={1}>
              ...
              <Box flexDirection={split ? 'row' : 'column'} gap={split ? 2 : 1}>
                <Box flexDirection='column' width={split ? 28 : undefined} flexShrink={0} gap={1}>
                  <Text color={theme.muted}>
                    {search !== undefined
                      ? t('${count} MATCHING ACTIONS', { count: String(items.length) })
                      : t(sections[section]).toUpperCase()}
                  </Text>
                  <Menu
                    choices={items.map((item, value) => ({
                      name: search !== undefined ? `${item.label} · ${t(item.section)}` : item.label,
                      value,
                    }))}
                    ...
                    limit={Math.max(
                      ...
                      Math.min(
                        8,
                        panelHeight - (rows >= 24 ? 4 : 2) - (search !== undefined ? 2 : 0) - (split ? 3 : 7),
                      ),
                    )}
        ...
        <Text color={state.title && !state.busy ? statusColor : theme.muted} wrap='truncate-end'>
          {state.busy
            ? state.prompt
              ? t('Pending form values are saved only when the form is complete.')
              : t('PicList is working on your request.')
            : state.title
              ? `${state.error ? '!' : state.outcome === 'cancelled' ? '–' : '✓'} ${state.title}: ${state.status}`
              : configured
                ? t('Ready when you are.')
                : t('Start with “Set up a destination” to connect your first uploader.')}
        </Text>
        <Box flexWrap='wrap'>
          {state.busy ? (
            <Shortcut keys='Ctrl+C' label={t('exit after active work')} />
          ) : showResults || showError ? (
            <>
              <Shortcut keys='↑↓' label={t('select result')} />
              <Shortcut keys='Esc' label={t('back')} />
            </>
          ) : (
            <>
              <Shortcut keys='↑↓' label={t('move')} />
              <Shortcut keys='Enter' label={t('open')} />
              {search === undefined ? (
                <>
                  <Shortcut keys='Tab' label={t('section')} />
                  <Shortcut keys='/' label={t('search')} />
                  {state.results.length > 0 && <Shortcut keys='r' label={t('results')} />}
                  <Shortcut keys='q' label={t('quit')} />
                </>
              ) : (
                <Shortcut keys='Esc' label={t('close search')} />
              )}
            </>
          )}
        </Box>
